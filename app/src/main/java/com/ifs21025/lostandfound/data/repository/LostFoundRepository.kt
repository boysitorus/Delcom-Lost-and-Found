...
class LostFoundRepository private constructor(
    ...
    fun getLostFounds(
        ...
        try {
            //get success message
            emit(MyResult.Success(apiService.getTodos(isCompleted, isMe, status)))
        } catch (e: HttpException) {
            //get error message
            val jsonInString = e.response()?.errorBody()?.string()
            emit(
    ...
    fun getLostFound(
        ...
        try {
            //get success message
            emit(MyResult.Success(apiService.getTodo(lostFoundId)))
        } catch (e: HttpException) {
            //get error message
            val jsonInString = e.response()?.errorBody()?.string()
            emit(
    ...
    fun deleteLostFound(
        ...
        try {
//get success message
            emit(MyResult.Success(apiService.deleteTodo(lostFoundId)))
        } catch (e: HttpException) {
            //get error message
            val jsonInString = e.response()?.errorBody()?.string()
            emit(
